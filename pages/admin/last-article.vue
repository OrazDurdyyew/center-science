  ...
  <div class="admin-journal">
    <div class="admin-journal__header">
      <div class="admin-journal__map">
        <nuxt-link to="/admin/last-article">Häzirki goýberiliş</nuxt-link>
      </div>
    </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in journals" @click="$router.push(`/admin/articles/${item.uuid}`)" :key="item.uuid">
              ...
              <td>{{ item.content }}</td>
              <td><img @click="openPdf(item.bigPath, true)" style="width: 20px; cursor: pointer" src="@/assets/icons/eye.png" alt="" /></td>
