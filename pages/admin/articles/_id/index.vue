  ...
  <div class="admin-journal">
    <div class="admin-journal__header">
      <div class="admin-journal__map">
        ...
        <span>/</span>
        <nuxt-link :to="`/admin/articles/${$route.params.id}`"
          >Makalalar
        </nuxt-link>
      </div>
      <div class="admin-journal__button">
        <button @click="$router.push(`/admin/articles/${$route.params.id}/0`)">
          + Täze Makala
        </button>
      </div>
    </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in journals" :key="item.uuid">
              <td>{{ index + 1 }}</td>
              <td>
                {{ item.authorName?.map((item) => item.name).join(", ") }}
              </td>
              <td>{{ item.title }}</td>
              <td>
                <img
                  @click="openPdf(item.path, false)"
              ...
              <td class="table__image">
                <img
                  @click="
                    $router.push(
                      `/admin/articles/${$route.params.id}/${item.uuid}`
                    )
                  "
              ...
              <td class="table__image">
                <img
                  @click="deleteJournal(item.uuid, false)"
...
export default {
  ...
  methods: {
    async fetchJournal() {
      try {
        const { data, status } = (
          await this.$axios.post("admin/get-articles", {
