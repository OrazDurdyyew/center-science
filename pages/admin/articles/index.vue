  ...
  <div class="admin-journal">
    <div class="admin-journal__header">
      ...
      <div class="admin-journal__dropdown">
        <drop-down :items="journals" :selectedItem="selectedJournal" @selectItem="selectListener"></drop-down>
      </div>
    </div>
    <div class="admin-journal__body">
      <div class="admin-journal__body-table">
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in journals" @click="$router.push(`/admin/articles/${item.uuid}`)" :key="item.uuid">
              <td>{{ index + 1 }}</td>
              <td>{{ new Date(item.dateFormat).toLocaleDateString() }}</td>
              <td>{{ item.content }}</td>
              <td><img @click.stop="openPdf(item.bigPath, true)" style="width: 20px; cursor: pointer" src="@/assets/icons/eye.png" alt="" /></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      async fetchJournal() {
        try {
          const { data, status } = (await this.$axios.post('admin/get-all', { page: 1, limit: 500 })).data
          if (status) {
            this.journals = data.data.filter((item) => (item.name = item.dateFormat))
          }
        } catch (error) {
          console.log(error)
        }
